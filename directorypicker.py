...
def st_directory_picker(initial_path=Path(), prompt = "Select a directory", base = st):

    if "path" not in st.session_state:
        st.session_state.path = initial_path.absolute()

    base.text_input(prompt, st.session_state.path)

    _, col1, col2, col3, _ = base.columns([3, 1, 3, 1, 3])

    with col1:
        base.markdown("#")
        if base.button("⬅️") and "path" in st.session_state:
    ...
    with col2:
        ...
        if subdirectroies:
            base.session_state.new_dir = base.selectbox(
